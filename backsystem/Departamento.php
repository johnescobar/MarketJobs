...
switch ( SIMNet::req( "action" ) )
{
	...
	case "insert" :
		$_POST["param"]["departamento"]["IDPais"] = SIMNet::post( "IDPais" );
	
		//seguridad para cada campo del formulario
		foreach($_POST["param"]["departamento"] as $clave=>$valor)
		{
			
			$_POST["param"]["departamento"][$clave] = SIMUtil::antiinjection($valor);
		}
		/*
		...
		if( !SIMNotify::capture( SIMUtil::valida( $_POST["param"]["departamento"] , $array_valida ) , "error" ) )
		{
			//los campos al final de las tablas
			$frm = SIMUtil::varsLOG( $_POST["param"]["departamento"] );
		...
		print_form( $_POST["param"]["departamento"] , "insert" , "Agregar Registro" );
		break;
	...
	case "update" :
		...
		print_form( $_POST["param"]["departamento"] , "update" ,  "Realizar Cambios" );
		break;
	...
	case "DelImgNot":
		$doceliminar = IMGNOTICIA_DIR.$dbo->getFields( "Noticia" , "NoticiaFile" , "IDNoticia = '" . $_GET[id] . "' " );
		unlink($doceliminar);
...
<table class="adminform">
	...
	<tr>
		...
		<table cellspacing="0" cellpadding="0" border="0" width="100%">
			...
			<tr>
				<td>Nombre</td>
				<td><input id="param[departamento][Nombre]" type="text" size="25"
					title="Nombre" name="param[departamento][Nombre]"
					class="input mandatory" value="<?php echo $frm["Nombre"] ?>" /></td>
			</tr>
           
			<tr>
				<td>Publicar</td>
				<td><?php echo SIMHTML::formRadioGroup( array_flip( SIMResources::$sino ) , $frm["Publicar"] , "param[departamento][Publicar]" , "title=\"Publicar\"" )?>
				</td>
			</tr>
			<tr>
